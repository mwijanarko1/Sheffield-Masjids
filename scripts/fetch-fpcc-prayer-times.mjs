#!/usr/bin/env node
/**
 * Fetches Firth Park Cultural Centre prayer data from mosqueprayertimes.org (same stack as
 * Emaan Trust / Grand Mosque): adhan (`prayers-ajax-list`) and iqamah (`mosques-ajax-list`).
 * Uses POST + Laravel CSRF + session cookie.
 *
 * The live HTML embeds `mosque-id: '11'` in AJAX snippets, but API rows return `mosque_id: 47`;
 * we send `mosque-id: 47` to match the API.
 *
 * Flow:
...
const BASE = "https://fpcc.mosqueprayertimes.org";
/** Confirmed from API JSON rows (`mosque_id`); differs from embedded script on the page. */
const MOSQUE_ID = "47";
const SLUG = "firth-park-cultural-centre";
