...
def build_daily_iqamah(prayer_times, iqamah_ranges):
...
def main():
    out_dir = Path("public/data/mosques/gb/leicester/islamic-dawah-academy")
    out_dir.mkdir(parents=True, exist_ok=True)

    prayer_times = USER_DATA["prayer_times"]
    iqamah_times = build_daily_iqamah(prayer_times, USER_DATA["iqamah_times"])

    output = {
        "month": "JULY",
        "prayer_times": prayer_times,
        "iqamah_times": iqamah_times,
        "jummah_iqamah": " / ".join(USER_DATA["jummah_iqamah"]),
    }

    (out_dir / "july.json").write_text(json.dumps(output, indent=2) + "\n")
    print(f"Wrote {out_dir / 'july.json'}")
    print(f"Jummah: {output['jummah_iqamah']}")
