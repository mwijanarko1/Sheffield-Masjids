...
test("buildMonthlyCalendarEvents exports adhan events including Sunrise in adhan mode", async () => {
  const events = await buildMonthlyCalendarEvents({ ...baseRequest, mode: "adhan" });

  assert.equal(events.length, 6);
  assert(events.every((event) => event.kind === "adhan"));
  assert.equal(events[0]?.title, "Fajr Adhan - Test Masjid");
  assert.equal(events[1]?.title, "Sunrise (Shuruq) Adhan - Test Masjid");
  ...
  assert.equal(events[1]?.start.toISOString(), "2026-04-01T06:37:00.000Z");
});

test("buildMonthlyCalendarEvents exports only concrete iqamah events in iqamah mode", async () => {
  const events = await buildMonthlyCalendarEvents({ ...baseRequest, mode: "iqamah" });
...
test("buildMonthlyCalendarEvents exports both adhan and iqamah without placeholders", async () => {
  const events = await buildMonthlyCalendarEvents({ ...baseRequest, mode: "both" });
  ...
  const firstFajrAdhan = events.find((event) => event.kind === "adhan" && event.title.startsWith("Fajr Adhan"));
  assert.equal(firstFajrAdhan?.uid, "test-masjid-2026-04-01-fajr-adhan@sheffieldmasjids");
