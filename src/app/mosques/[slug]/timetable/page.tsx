...
export async function generateMetadata({
  ...
  const title = `${mosque.name} Monthly Timetable in ${mosque.cityName}`;
  const fullTitle = `${title} | ${SITE_NAME}`;
  const description = `Full monthly prayer times and iqamah times for ${mosque.name} in ${mosque.cityName}.`;
...
export default async function TimetablePage({ params }: TimetablePageProps) {
  ...
  return (
    <main className="relative min-h-[100dvh] sm:min-h-screen text-[var(--theme-fg,#fff)]">
